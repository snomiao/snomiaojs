...
export const 不晚于 = (乘数: number) => ({
    ...
    月前: ({ $not: { $gt: new Date(+new Date().getTime() - 30 * 86400e3 * 乘数) } }),
    年前: ({ $not: { $gt: new Date(+new Date().getTime() - 365 * 86400e3 * 乘数) } }),
})
...
export const 不早于 = (乘数: number) => ({
    ...
    月前: ({ $not: { $lt: new Date(+new Date().getTime() - 30 * 86400e3 * 乘数) } }),
    年前: ({ $not: { $lt: new Date(+new Date().getTime() - 365 * 86400e3 * 乘数) } }),
})
